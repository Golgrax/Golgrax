#!/usr/bin/env node
// Generates a modern SVG showing total repository count for a user.
// No language filtering — counts all repositories (public + private when token has scopes).
// Node 14+ (Actions runners include Node 18).
...
if (!username) {
  console.error('Missing GH_USERNAME or repository owner.');
  process.exit(2);
}

if (!token) {
  console.error('Missing GH_API_TOKEN / GITHUB_TOKEN environment variable. Set secret GH_PAT (recommended) to include private repos.');
  process.exit(2);
}
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    try {
      ...
      const total = json.data && json.data.user && json.data.user.repositories && json.data.user.repositories.totalCount;
      if (typeof total !== 'number') {
        console.error('Unexpected GraphQL response', body);
        process.exit(4);
      }
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="title desc">
  <title id="title">${user} - ${label} badge</title>
  <desc id="desc">Shows the total repository count for ${user} on GitHub</desc>
  <defs>
  ...
  <g transform="translate(22,22)" fill="#fff" font-family="system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial">
    ...
    <text x="72" y="30" font-size="22" font-weight="700" fill="#fff">${user}</text>
